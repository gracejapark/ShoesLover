...
function confirmOrders() {
	document.getElementById('operation').value = 'confirmOrders';
	document.getElementById('proceedOrders').submit();
}

function cancelOrders() {
	document.getElementById('operation').value = 'cancelOrders';
	document.getElementById('proceedOrders').submit();
}
</script>
...
<p align="center"><font color="8000000" size="6"><%="CHECK OUT" %></font></p>
<h2></h2>

<form method="post" id='proceedOrders' action="OrdersServlet">
	<input type="hidden" name="operation" id="operation" value="confirmOrders" />
<%
    ...
    String ORDER_STATUS_INPLACE = "Order Placed";
    
	orderbean.setItemid(Integer.valueOf(request.getParameter("selectedShoeId")));
    orderbean.setCustid(customerbean.getCustid());
    orderbean.setStatus(ORDER_STATUS_INPLACE);
    ...
    if(orderdao.addData(orderbean)) {
    	orders = orderdao.listOrderData(customerbean.getCustid(), ORDER_STATUS_INPLACE);
    } else {
    	// exception occurred during adding new item
    }
%>  
...
<p align="center">
	<input type="button" onClick="confirmOrders();" value="Comfirm"/> 
	<input type="button" onClick="cancelOrders();" value="Cancel all orders"/>
</p>
</form>
</body>
</html>